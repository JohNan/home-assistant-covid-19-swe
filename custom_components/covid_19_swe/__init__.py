"""The COVID-19 Sweden integration."""
import asyncio
from datetime import timedelta
import logging

import async_timeout
from .covid_19_swe import get_cases
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    if isinstance(entry.data["region"], int):
        hass.config_entries.async_update_entry(
            entry, data={**entry.data, "region": entry.title}
        )
    ...
    if not entry.unique_id:
        hass.config_entries.async_update_entry(entry, unique_id=entry.data["region"])
